...
public class ConfigServiceRegistry {
	...
	private final OrderedServiceDelagator<ConfigService> orderedServiceDelagator;
	...
	private ConfigServiceRegistry() {
		...
		orderedServiceDelagator = new OrderedServiceDelagator<>(configServiceLoader);
	...
	private synchronized void initialize() {
		if (!initialized) {
			for (Iterator<ConfigService> it = orderedServiceDelagator.iterator(); it.hasNext(); ) {
	...
	public <T extends IApplicationConfig> T loadConfig(String filename) {
		...
		T result = null;
		for (Iterator<ConfigService> it = orderedServiceDelagator.iterator(); result == null && it.hasNext(); ) {
	...
	public <T extends IApplicationConfig> void saveConfig(String applicationConfigFilename, T applicationConfig) {
		...
		for (Iterator<ConfigService> it = orderedServiceDelagator.iterator(); !saved && it.hasNext(); ) {
			ConfigService next = it.next();
			try {
